...
CREATE TABLE Delivery_new(
	...
	LabelDamage FLOAT NULL,
	Damage FLOAT NULL,
	ReturnsCollected FLOAT NULL,
...
SELECT w.WarehouseLocation, AVG(d.LabelDamage) AS avg_label_damage, AVG(d.Damage) AS avg_parcel_damage
FROM Delivery d
JOIN Warehouse w ON d.WarehouseID = w.WarehouseID
GROUP BY w.WarehouseLocation
HAVING AVG(d.LabelDamage) > (SELECT avg_overall_label_damage FROM #var_temp_table WHERE avg_overall_label_damage IS NOT NULL)
AND AVG(d.Damage) > (SELECT avg_overall_parcel_damage FROM #var_temp_table WHERE avg_overall_parcel_damage IS NOT NULL)
...
SELECT AVG(ReturnsCollected)
FROM Delivery
WHERE LabelDamage > 0 AND Damage > 0;

SELECT s.StoreName, AVG(d.LabelDamage) AS avg_label_damage, AVG(d.Damage) AS avg_parcel_damage, AVG(d.ReturnsCollected) AS avg_returns_collected
FROM Delivery d
JOIN Store s ON s.StoreID = d.StoreID
GROUP BY s.StoreName
HAVING AVG(d.ReturnsCollected) > (SELECT avg_overall_returns FROM #var_temp_table WHERE avg_overall_returns IS NOT NULL) 
AND AVG(d.LabelDamage) > (SELECT avg_overall_label_damage FROM #var_temp_table WHERE avg_overall_label_damage IS NOT NULL) 
AND AVG(d.Damage) > (SELECT avg_overall_parcel_damage FROM #var_temp_table WHERE avg_overall_parcel_damage IS NOT NULL)
ORDER BY StoreName;
...
WITH 
	...
	SELECT matchmethod_sum_cte.StoreID, matchmethod_sum_cte.MatchMethod, matchmethod_sum_cte.sum_match_method, store_sum_match_method_cte.store_match_method, ROUND((CAST(matchmethod_sum_cte.sum_match_method AS DECIMAL(5,0)) / CAST(store_sum_match_method_cte.store_match_method AS DECIMAL(5,0))) * 100, 0) AS percentage_of_store
...
-- Unable to use CTE in this query as ORDER BY clause does not work in CTE

SELECT MatchMethod, AVG(percentage_of_store) AS mean_percentage_of_store, 
	(((SELECT MAX(bot_half_max)
	FROM (SELECT TOP 50 PERCENT percentage_of_store AS bot_half_max
		FROM #match_method_dist_by_store
		ORDER BY percentage_of_store
		) AS bottom_half) +
	(SELECT MIN(top_half_min)
	FROM (SELECT TOP 50 PERCENT percentage_of_store AS top_half_min
		FROM #match_method_dist_by_store
		ORDER BY percentage_of_store DESC
		) AS top_half)) / 2) AS median_percentage_of_store,
